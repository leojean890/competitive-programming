...
public class RollingBalls {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<LittleState> bestStates = new ArrayList<>();
        ...
        if (N < 9) {
            ...
            while ((System.currentTimeMillis()- startTime) < 2000) {
                //nRollouts++;
                List<String> moves = new ArrayList<>();
                List<LittleState> currStates = new ArrayList<>();
                int[][] currentGrid = new int[N][N];
                List<int[]> freeCells = new ArrayList<>();
                List<int[]> balls = new ArrayList<>();
                ...
                for (int nMove = 0; nMove < N * N; nMove++) {
                    ...
                    moves.add(y + " " + x + " " + dirForMove(dy, dx));
                    currStates.add(new LittleState(index, dir1ForMove(dy, dx)));
            ...
            List<int[]> currMoves = new ArrayList<>();
            List<LittleState> currStates = new ArrayList<>(bestStates);
            ...
            while (elapsed < 9000) {
                ...
                List<LittleState> states = new ArrayList<>(currStates);
                int[][] currentGrid = new int[N][N];
                ...
                while (i < L) {
                    ...
                    if (i == currMove && actionType > 0) {
                        ...
                        if (actionType == 1) {
                            states.set(i, new LittleState(toMove,direc));
                        } else {
                            states.add(i, new LittleState(toMove,direc));
                            L += 1;
                        }
                    }

                    LittleState state = states.get(i);
                    ...
                    if(!(a >= 0 && a < N && b >= 0 && b < N && currentGrid[a][b] == 0)) {
                        ...
                        if (i == currMove && L == i) {
                            ...
                            dy = DIRS.get(direc)[0];
                            dx = DIRS.get(direc)[1];
                            states.add(new LittleState(toMove,direc));
                            L += 1;
                        }
                        continue;
                    }
    ...
    public static class LittleState {
        int index;
        int direction;

        public LittleState(int index, int direction) {
            this.index = index;
